...
class Node:
    ...
    def visit(self, visited):
        ...
        if self.name == "start" and "start" in visited:
            return 0

        paths = 0
        twosmall = False
        for n in set(filter(lambda x: x.islower() and x != "start" and x != "end", visited)):
            if visited.count(n) == 2:
                twosmall = True
                break

        if self.name.islower() and self.name in visited and twosmall:
            return 0
        else:
